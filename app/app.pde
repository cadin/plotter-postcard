...
void setup() {
	...
	float[] sizes = {messagePenThickness, addressPenThickness, captionPenThickness, dividerPenThickness};
	imgSaver = new ImageSaver(sizes);
...
void draw() {
	...
	if(saveLayers && imgSaver.state == SaveState.SAVING && imgSaver.mode == SaveMode.SVG) {
		sketch.drawLayersWithPenSize(imgSaver.penSize);
	} else {
		sketch.draw();
	}
...
float calculateStrokeSize(float mm) {
	return (mm * 0.03937008) * printResolution * screenScale; 
}
...
class ImageSaver {
	...
	ImageSaver(float[] sizes) {
		penSizes = new ArrayList<Float>();
		for(float s : sizes) {
			penSizes.add(s);
		}
	}
	...
	void updateSaveMode() {
		if(mode == SaveMode.SVG){
			do {
				penSizeIndex++;
				penSize = penSizes.get(penSizeIndex);

			} while((penSizeIndex < penSizes.size() - 1) && penSizes.indexOf(penSizes.get(penSizeIndex)) < penSizeIndex);

			if(penSizeIndex < penSizes.size() - 1) {
				return;
			}
	...
	void startSave() {
		if(state == SaveState.SAVING) {
			if(mode == SaveMode.SVG){
				...
				beginRecord(SVG, "output/" + filename + "-" + penSize + ".svg");
			}
		}
	}
	...
	void resizeSVG(String filename, float wInches, float hInches, int wPx, int hPx){
		print("- resizing SVG... ");
		String[] lines = loadStrings("output/" + filename + "-" + penSize + ".svg");
		...
		saveStrings("output/" + filename + "-" + penSize + ".svg", lines);
		println("done!");
	}
